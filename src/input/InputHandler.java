...
public class InputHandler {
	...
	private long availableJoysticks = 0;

	public InputHandler() {
		for (int i = GLFW.GLFW_JOYSTICK_1; i <= GLFW.GLFW_JOYSTICK_LAST; i++) {
			if (GLFW.glfwJoystickPresent(i)) {
				availableJoysticks |= 1 << i;
			}
		}
			...
			public void invoke(long window, double xpos, double ypos) {
				...
				for (InputContext context : contexts) {
					if (context.getMouseMovementInputTypeFor(Axis.HORIZONTAL) == InputType.RANGE) {
						context.addRangeValue(context.getMouseMovementAction(Axis.HORIZONTAL),
								mouseMovedDistance.x * mouseSensitivity);
						break;
					}
				}
				for (InputContext context : contexts) {
					if (context.getMouseMovementInputTypeFor(Axis.VERTICAL) == InputType.RANGE) {
						context.addRangeValue(context.getMouseMovementAction(Axis.VERTICAL),
								mouseMovedDistance.y * mouseSensitivity);
						break;
					}
				}
			}
	...
	public void update() {
		for (int joystick = GLFW.GLFW_JOYSTICK_1; joystick  <= GLFW.GLFW_JOYSTICK_LAST; joystick ++) {
			if(((availableJoysticks >> joystick) & 1) == 0){
				continue;
			}
